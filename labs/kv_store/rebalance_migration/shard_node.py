#!/usr/bin/env python3
"""
Shard node con supporto alla migrazione.
"""
    ...
    def handle(self, raw_line: str) -> str:
        ...
        with self._lock:
            ...
            if operation == "LIST_ITEMS":
                return json.dumps(
                    {"status": "OK", "shard": self._shard_id, "items": dict(sorted(self._data.items()))}
                )

            if operation == "IMPORT_KEY":
                self._data[key] = str(message.get("value", ""))
                self._migration_count += 1
                return json.dumps({"status": "OK", "shard": self._shard_id, "imported": key})

            if operation == "DELETE_LOCAL":
                self._data.pop(key, None)
                return json.dumps({"status": "OK", "shard": self._shard_id, "deleted": key})
...
def handle_connection(connection: socket.socket, address: tuple[str, int], shard: MigrationShard) -> None:
...
def serve() -> None:
    args = parse_args()
    shard = MigrationShard(args.shard_id)

    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        server_socket.bind((args.host, args.port))
        server_socket.listen()
        log(f"migration shard {args.shard_id} listening on {args.host}:{args.port}")
